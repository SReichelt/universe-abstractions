...
largely intended as documentation for the accompanying formalization in Lean 4\cite{Lean}.
...
calculus\cite{Simply-typed lambda calculus} because functors corresponding to the $\nm{S}$ and
$\nm{K}$ combinators exist (theorem \ref{thm:funop}).
...
The framework is influenced by ideas from Homotopy Type Theory\cite{HoTT}, but it is built on
...
implemented in Lean has been proved to be equiconsistent with ZF plus choice and
universes\cite{Lean-ZFC}, it is safe to assume that analogous constraints are sufficient in
the latter.
...
\begin{proposition}
  ...
  In this case, $\U^A$ also has embedded functors, defined by $(B^A \to C^A) := (B \to C)^A$
  for $B,C \in \U.$
\end{proposition}
\vspace{-1ex}
\begin{proof}
...
not have full functor operations, then $\U^A$ does not have embedded functors, but
...
\begin{proposition}
  For a universe $\U$ with embedded functors and at least linear functor operations,
  $\U^{A?}$ has embedded functors as follows. For $B,C \in \U,$ we set
  \begin{itemize}
...
\begin{theorem}
  ...
  Embedded functors in a universe $\U$ with full functor operations (or unconditionally
...
\begin{remarks}
  ...
  combinator calculus follows from five axioms is given in \cite{Extensionality},
